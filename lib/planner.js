...
function generateOneDayPlan(weight, state, dietKey, catKey, dalName, numMeals, foods, usedStateNonveg, supplements) {
  ...
  // Stage 6: Supplement integration
  // For each supplement, run a priority-reduction loop to offset the supplement protein
  // from the plan's natural protein foods (25g at a time, in priority order), then
  // inject the supplement item directly into the breakfast slot.
  if (supplements && supplements.length > 0) {
    const totalProteinToOffset = supplements.reduce((s, sup) => s + (sup.perServing?.protein || 0), 0)

    if (totalProteinToOffset > 0) {
      // Build priority list [foodName, slotName] based on diet type and category
      const soySlot = hasBf ? 'Breakfast' : 'Lunch'
      const pList = [['Cooked Soy Chunks (TVP)', soySlot]]

      if (dietKey === 'vegetarian') {
        // Cat A: breakfast=Soy, lunch=Tofu, dinner=Paneer
        // Cat B: breakfast=Soy, lunch=Paneer, dinner=Tofu
        // Priority 2 = Paneer, Priority 3 = Tofu (per user spec)
        const paneerSlot = catKey === 'A' ? 'Dinner' : 'Lunch'
        const tofuSlot   = catKey === 'A' ? 'Lunch'  : 'Dinner'
        pList.push(['Low Fat Paneer', paneerSlot])
        pList.push(['Firm Tofu', tofuSlot])
      } else if (dietKey === 'vegetarian_egg') {
        // Cat A: breakfast=Soy, lunch=Egg, dinner=Paneer
        // Priority 2 = Paneer (veg), Priority 3 = Egg
        pList.push(['Low Fat Paneer', 'Dinner'])
        pList.push(['Whole Egg (Boiled)', 'Lunch'])
      } else {
        // non_vegetarian A: dinner=Paneer, non_vegetarian B: dinner=Tofu
        // Priority 2 = veg dinner protein, Priority 3 = non-veg lunch protein
        const vegDinnerProt = catKey === 'A' ? 'Low Fat Paneer' : 'Firm Tofu'
        pList.push([vegDinnerProt, 'Dinner'])
        pList.push([lpName, 'Lunch'])
      }

      // Reduce 25g from each source in priority order until protein target is met.
      // Check after every individual 25g step.
      let accumulated = 0
      const STEP = 25
      let maxRounds = 20 // safety cap to prevent infinite loops
      suppLoop: while (accumulated < totalProteinToOffset && maxRounds-- > 0) {
        let anyReduced = false
        for (const [fn, sn] of pList) {
          if (!(sn in meals)) continue
          const qty = getQty(sn, fn)
          if (qty < STEP) continue
          const proteinGained = r((foods[fn]?.protein || 0) * STEP / 100)
          removeItem(sn, fn, STEP)
          accumulated = r(accumulated + proteinGained)
          anyReduced = true
          if (accumulated >= totalProteinToOffset) break suppLoop
        }
        if (!anyReduced) break // no food could be further reduced
      }

      // Inject supplement items directly into the breakfast slot (or Lunch for 2-meal plans)
      const targetSlot = hasBf ? 'Breakfast' : (('Lunch' in meals) ? 'Lunch' : slots[0][0])
      for (const sup of supplements) {
        if (!sup.perServing) continue
        const name = sup.type === 'protein_powder' ? 'Protein Powder' : 'Protein Bar'
        let qty, unit
        if (sup.macroBasis === 'per100g') {
          qty  = sup.servingGrams || 30
          unit = 'g'
        } else {
          qty  = 1
          unit = sup.type === 'protein_bar' ? ' bar' : ' scoop'
        }
        const m = {
          calories: r(sup.perServing.calories),
          protein:  r(sup.perServing.protein),
          carbs:    r(sup.perServing.carbs),
          fat:      r(sup.perServing.fat),
          fibre:    r(sup.perServing.fibre),
        }
        meals[targetSlot].items.push({ name, qty, unit, macros: m })
        meals[targetSlot].totals = addMacros(meals[targetSlot].totals, m)
      }
    }
  }
...
export function generate7DayPlan(weight, state, dietType, numMeals, foods, supplements) {
  ...
  const weeklyPlan=rotation.map(([ck,edk],idx)=>{
    const d=generateOneDayPlan(weight,state,edk,ck,dalOrder[idx],numMeals,foods,usedNV,supplements||[])
